...
let PDFParser, pdfToPng;
try {
  PDFParser = require('pdf2json');
} catch (e) {
  console.warn('Warning: pdf2json module not loaded.');
}
try {
...
// pdf2json 기반 안전한 PDF 텍스트 추출 헬퍼 함수 (전역 pdfjsLib 오염 0%)
function extractPdfTextWithPdf2json(filePath) {
  return new Promise((resolve) => {
    if (!PDFParser || !fs.existsSync(filePath)) {
      return resolve('');
    }
    try {
      const pdfParser = new PDFParser(null, 0); // 1 = raw text mode    0 = normal mode (Warning 출력 차단)
      pdfParser.on("pdfParser_dataError", (errData) => {
        console.error('pdf2json extraction error:', errData && errData.parserError ? errData.parserError : errData);
        resolve('');
      });
      ...
      console.error('pdf2json load error:', e.message);
      resolve('');
    }
  });
}
...
async function evaluatePdfSubmission(assignment, filePath, fileName) {
  ...
  if (apiKey && ChatOpenAI) {
    try {
      ...
      // PDF 텍스트 원문 파싱 (pdf2json 사용으로 전역 pdfjsLib 충돌 원천 차단)
      let extractedPdfText = '';
      if (PDFParser && fs.existsSync(filePath)) {
        try {
          extractedPdfText = await extractPdfTextWithPdf2json(filePath);
        } catch (parseErr) {
...
\`\`\`
      ...
      if (pdfToPng && HumanMessage && fs.existsSync(filePath)) {
        try {
          // pdf-parse와 pdf-to-png-converter 간 전역 pdfjsLib 버전 충돌(5.4.296 vs 6.2.108) 초기화
          delete global.pdfjsLib;
          delete global._pdfjsTestingUtils;

          // Buffer 객체 전달 및 disableFontFace 옵션으로 Canvas font clip 에러 방지
